...
### 1. 키스토어 파일 생성 (프로젝트당 최초 1회만)

Release APK를 빌드하기 위해서는 서명용 키스토어 파일이 필요합니다.

**⚠️ 중요:** 키스토어 파일은 **프로젝트당 하나만** 생성합니다. 여러 컴퓨터에서 같은 키스토어 파일을 공유해야 합니다.

#### 키스토어가 이미 있는 경우

팀원이 이미 키스토어 파일을 생성했다면, 다음 파일들을 받아서 `android/app/` 폴더에 복사하세요:

- `jellypaw-release.keystore` (키스토어 파일)
- 키스토어 비밀번호 정보
...
- 키스토어 비밀번호: 원하는 비밀번호 입력 (노션 프론트 메모장 참고)
- 이름, 조직 등 정보 입력
- 키 비밀번호: 키스토어 비밀번호와 동일하게 하려면 Enter

**⚠️ 중요 사항:**

- ✅ 키스토어 파일은 **프로젝트당 하나만** 생성 (컴퓨터마다 생성하지 않음)
- ✅ 같은 키스토어로 서명해야 앱 업데이트가 가능
- ✅ 키스토어 파일과 비밀번호는 안전하게 보관 (백업 필수)
- ✅ 분실 시 앱 업데이트 불가능
- ✅ Git에는 커밋하지 않음 (`.gitignore`에 포함됨)
