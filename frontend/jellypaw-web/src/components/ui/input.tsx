...
const Input = React.forwardRef<HTMLInputElement, React.ComponentProps<'input'>>(({ className, type, ...props }, ref) => {
  const inputElement = (
    <input
      type={type}
      className={cn(
        'flex h-12 rounded-2xl border border-input bg-white-100 py-3 p2 placeholder:text-gray-300 placeholder:p2 focus:outline-none focus-visible:outline-none focus-visible:ring-0',
        type === 'search'
