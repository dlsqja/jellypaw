...
const Input = React.forwardRef<HTMLInputElement, JellyInputProps>(
  ({ className, type, errorText, helperText, helperTextClassName, containerClassName, ...props }, ref) => {
    ...
    const baseInputClass =
      'flex h-12 rounded-[12px] border border-input bg-white-100 py-3 p2 placeholder:text-gray-300 placeholder:p2 placeholder:leading-[1.5] placeholder:font-normal focus:outline-none focus-visible:outline-none focus-visible:ring-0.5';

    const normalFocusClass = 'focus-visible:border-aqua-300 focus-visible:ring-aqua-300';
    const errorFocusClass = 'border-[#e85555] focus-visible:border-[#e85555] focus-visible:ring-[#e85555]/30';
    ...
    const inputElement = <input ref={ref} type={type} className={inputClassName} {...props} />;
    ...
    return (
      <div className={cn('w-full', containerClassName)}>
        ...
        {showError && <p className="mt-1.5 text-xs text-[#e85555]">{errorText}</p>}

        {showHelper && <p className={cn('mt-1.5 text-xs text-[#999999]', helperTextClassName)}>{helperText}</p>}
      </div>
    );
  },
);
