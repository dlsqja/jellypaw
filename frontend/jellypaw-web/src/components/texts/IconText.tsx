...
const IconText = React.forwardRef<HTMLDivElement, IconTextProps>(
  ({ className, icon: Icon, label, asChild = false, size, iconTone, textTone, textStyle, iconClassName, labelClassName, ...props }, ref) => {
    ...
    return (
      <Comp ref={ref as any} className={cn(rootVariants({ size }), className)} {...props}>
        ...
        <span className={cn(labelVariants({ textTone, textStyle }), labelClassName)}>{label}</span>
      </Comp>
    );
  },
);
